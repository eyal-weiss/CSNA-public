...
\subsection{Step 2: Edge Cost --- Observed Divergence $g_{ij}$ (Equation 3)}
\label{sec:gij}
...
\subsection{Step 3: The Sigmoid Concordance Score $s_{ij}$ (Equation 4)}
...
\textbf{What it computes:}
Converts the distance $g_{ij}$ into a ``concordance score'' between 0 and 1.
High score ($s_{ij} \approx 1$) means the edge is likely \textbf{concordant}
(same class); low score ($s_{ij} \approx 0$) means it is likely \textbf{discordant}
(different class).

\textbf{Symbol breakdown:}
\begin{itemize}
  \item $g_{ij}^{(\ell)} \geq 0$: the distance from Eq.~3.
...
in $(0, 1)$, which is needed for two reasons:
...
\subsection{Step 4: Dual-Channel Aggregation (Equations 5--6)}
...
\subsection{Step 5: The Gating Mechanism (Equation 7)}
\label{sec:gating}
...
On the Wisconsin and Actor datasets, the paper reports that CSNA learns
$\gamma_{\text{self}} = 1.0$ for all nodes, effectively ignoring the graph
entirely and reducing to an MLP.
\end{example}


\subsection{The Extended Variant: $f = g + h$ (Equation 8)}
\label{sec:extended}
...
concordance formula (Eq.~4).

\textbf{Symbol breakdown:}
\begin{itemize}
  \item $g_{ij}$: the observed divergence from Eq.~3 (L2 distance in projection space).
...
\subsection{Calibration Loss (Equation 9)}
...
The \textbf{calibration regularizer} (Eq.~9) encourages this by penalizing high costs
...
$s_{ij}$ & Scalar $\in (0,1)$ & Concordance score \\
