...
Under a contextual stochastic block model we show that cost-sensitive weighting preserves class-discriminative signal where mean aggregation provably attenuates it, provided $w_+/w_- > q/p$.
...
(3)~Under a contextual stochastic block model (CSBM), we prove that cost-sensitive weighting preserves class signal where mean aggregation degrades it, provided $w_+/w_- > q/p$ (\Cref{sec:theory}).
...
The separation in \Cref{fig:cost_dist} is strongest on Texas and Cornell (where CSNA outperforms GCN by 21+ pp) and weakest on Actor (where the margin is 8pp), suggesting that routing quality---the cost function's ability to discriminate same-class from cross-class edges---correlates with the performance gain from per-edge routing.
