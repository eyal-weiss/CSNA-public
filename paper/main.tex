...
After one round of mean aggregation $\mathbf{H}^{(1)} = \tilde{\mathbf{A}} \mathbf{X}$, where $\tilde{\mathbf{A}} = \mathbf{D}^{-1/2}(\mathbf{A}+\mathbf{I})\mathbf{D}^{-1/2}$ is the symmetrically normalized adjacency, the expected between-class separation satisfies:
\begin{equation}
...
We report classification accuracy (fraction of correctly labeled test nodes), averaged over the 10 splits, with standard deviation reflecting split-to-split variability.
