...
(3)~Empirical evaluation on six heterophily and three homophily benchmarks with uniform tuning across all methods (\Cref{sec:experiments}).
...
\begin{itemize}
    ...
    \item \textbf{Similarity to ACM-GNN.} CSNA shares the dual-channel-plus-gating design with ACM-GNN~\cite{luan2022revisiting}. The per-edge routing is a meaningful architectural difference, but accuracy gains over ACM-GNN are not consistently significant.
\end{itemize}
...
The key architectural difference from the closely related ACM-GNN is per-edge routing rather than uniform spectral channels---a finer-grained mechanism that, in our experiments, yields comparable accuracy at higher computational cost.
Our theoretical analysis provides a clean condition ($w_+/w_- > q/p$) under which cost-sensitive weighting improves over mean aggregation.
Future work includes adaptation to the informative-heterophily regime (where CSNA currently underperforms), efficient scaling via edge sampling, and extending the analysis beyond single-layer binary CSBM.
